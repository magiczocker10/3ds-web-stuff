window.addEventListener( 'load', function () {
	...
	function checkWin() {
		for ( var i = 0; i < size; i++ ) {
			if ( getSumOfColOrRow( i, false ) !== sizes[ size ][ 2 ] ) {
				return false;
			}
			if ( getSumOfColOrRow( i, true ) !== sizes[ size ][ 2 ] ) {
				return false;
			}
		}
		alert( 'You won!' );
	...
	function generateField( s ) {
		var a = '123456789'.slice( 0, Math.max( 0, s ) );
		a = a + a + a + a;
		for ( var i = 0; i < s; i++ ) {
			var offset = Math.ceil( ( i + 1 ) / s * sizes[ s ][ 0 ] ) - 1;
			field[ i ] = a.substring( i * sizes[ s ][ 0 ] + offset, i * sizes[ s ][ 0 ] + s + offset ).split( '' );
		}
	}
	...
	document.getElementById( 'board' ).addEventListener( 'click', function ( e ) {
		if ( e.target.nodeName !== 'TD' || e.target.className.indexOf( 'input' ) < 0 ) {
			return;
		}

		if ( last ) {
			last.className = last.className.replace( ' selected', '' );
		}
		last = e.target;
